...
AttachServer::AttachServer(const rclcpp::NodeOptions &options)
    : Node("attach_server_component", options) {
  ...
  lift_shelf_pub =
      this->create_publisher<std_msgs::msg::String>("elevator_up", qos_profile);
}
...
void AttachServer::shelf_leg_detection() {
...
void AttachServer::publish_cart_frame_tf() {
...
void AttachServer::move_cart_legs_center() {
...
void AttachServer::move_under_cart() {
  RCLCPP_INFO(this->get_logger(),
  ...
  try {
    start_tf =
        tf_buffer_->lookupTransform(odom_frame, base_frame, tf2::TimePointZero);
  } catch (const tf2::TransformException &ex) {
